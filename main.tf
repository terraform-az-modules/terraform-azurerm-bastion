...
resource "azurerm_bastion_host" "main" {
  ...
  file_copy_enabled   = contains(["Standard", "Premium"], var.bastion_host_sku) ? var.enable_file_copy : null
  sku                 = var.bastion_host_sku
  ip_connect_enabled   = contains(["Standard", "Premium"], var.bastion_host_sku) ? var.enable_ip_connect : null
  scale_units         = contains(["Standard", "Premium"], var.bastion_host_sku) ? var.scale_units : 2
  shareable_link_enabled = contains(["Standard", "Premium"], var.bastion_host_sku) ? var.enable_shareable_link : null
  tunneling_enabled      = contains(["Standard", "Premium"], var.bastion_host_sku) ? var.enable_tunneling : null
  tags                    = module.labels.tags
...
resource "azurerm_monitor_diagnostic_setting" "pip_diagnostic" {
  ...
  dynamic "enabled_log" {
    for_each = var.pip_logs.enabled ? coalesce(try(var.pip_logs.category, null), try(var.pip_logs.category_group, null), ["AllLogs"]) : []
    content {
